...
gameselect.onchange = (e) => draw(choosePlayer.value, gameselect.value);

choosePlayer.onchange = (e) => {
    d3.select('#game-select').selectAll('*').remove()
    fetch(`/api/playergamelog?DateFrom=&DateTo=&LeagueID=&PlayerID=${choosePlayer.value}&Season=2023-24&SeasonType=Regular+Season`)
        .then(res => res.json())
        .then( data => {
            ...
            tempOption.innerText = 'Select a game';
...
fetch('/api/playerindex?LeagueID=00&Season=2023-24')
    ...
    .then( res => {
        res['PlayerIndex'].forEach( e => {
            let option = document.createElement('option')
            option.innerText = e['PLAYER_FIRST_NAME'] + " " + e['PLAYER_LAST_NAME']
            option.value = e['PERSON_ID']
            choosePlayer.appendChild(option)
        })
    })
...
function draw(playerID, game) {
    ...
    drawCourt()
    fetch(`/api/shotchartdetail?ContextMeasure=FGA&Month=0&OpponentTeamID=0&Period=0&PlayerID=${playerID}&Season=2023-24&TeamID=0&GameID=${game}`)
    .then(resp => resp.json())
    .then(res => {
